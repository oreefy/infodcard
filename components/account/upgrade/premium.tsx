...
export default function Premium() {
  ...
  const plan = "PREMIUM_TO_BUSINESS";
  ...
  const confirm = async (event: any) => {
    ...
    if (method) {
      const data = {
        plan: "BUSINESS",
        price: plans[plan].price + (products?.price || 0),
        coupon: coupon,
  ...
  return (
    <>
      <div className="box p-0 overflow-hidden mb-0">
        <h1 className="text-2xl text-center font-bold px-3 py-4 bg-white/50">Upgrade to Business</h1>
