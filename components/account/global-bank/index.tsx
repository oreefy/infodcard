import Create from "@/components/account/global-bank/create";
import Update from "@/components/account/global-bank/update";
...
export default function GlobalBankPage() {
        ...
        Fetch("/api/account/profile/views", { method: "POST" }).then((res) => {
            if (res.status === 200) {
                if (res.body.profiles.length) {
                    ...
                    setGlobalBank(res.body.profiles[0].globalBanks);
                    setNewAccount(false);
    ...
    const deleteAction = (unique: string) => {
        setDeleted(unique);
        Fetch("/api/account/global-bank/delete", { method: "POST", body: { link: link, unique: unique } }).then((res) => {
            if (res.status === 200) {
                setDeleted("");
                loadFirst();
            } else {
    ...
    useEffect(() => {
        if (select) {
            setGlobalBank(profiles[select as any]?.globalBanks);
            ...
            <div>
                ...
                </div>}
                {
                    !loader && !newAccount && <div className="box p-0 mb-0">
                        <div className="box rounded-b-none flex gap-3 items-center justify-between m-0">
                            {creator && <h1 className="text-xl font-bold">{"Create Global Bank"}</h1>}
                            {updator && <h1 className="text-xl font-bold">{"Update Global Bank"}</h1>}
                            {!creator && !updator && <h1 className="text-xl font-bold">{`All Global Banks (${globalBank.length}/${db.limits.globalBank[session?.user?.name as "PREMIUM" | "BUSINESS"]})`}</h1>}
                        ...
                        <div className="p-3">
                            {!loader && !newAccount && creator ? <Create link={link} /> : (<div>
                                {!loader && !newAccount && globalBank && globalBank.length <= 0 && <div className="box mb-0">
                                    <h2 className="text-center py-10 px-4">No Global Bank Found.</h2>
                                </div>}
                                {!loader && !newAccount && globalBank && globalBank.length >= 1 && updator && <Update link={link} product={updator} />}
                                {!loader && !newAccount && globalBank && globalBank.length >= 1 && !updator && <>
                                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 w-full gap-3">
                                        {
                                            globalBank.map((bank, index) => {
                                                return <div key={index} className="rounded-xl bg-white/30 p-5">
                                                    <div className="grid grid-cols-5 gap-1 items-center justify-center">
                                                        ...
                                                        <div className="col-span-3 text-center">
                                                            <p className="font-bold text-xl bg-white/50 mb-2 px-2 py-1 rounded-lg break-words">{bank.lineOne}</p>
                                                            <p className="font-semibold text-xl bg-white/50 mb-2 px-2 py-1 rounded-lg break-words">{bank.lineTwo}</p>
                                                            <p className="font-semibold text-medium bg-white/50 mb-2 px-2 py-1 rounded-lg break-words">{bank.lineThree}</p>
                                                        </div>
                                                        <div className="col-span-2">
                                                            <Image className="w-full h-full object-cover object-center rounded-xl" src={bank.logo} width={100} height={100} alt={bank.bank}></Image>
                                                        </div>
