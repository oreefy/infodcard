...
export default async function DeleteMobileBankApi(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        if (await method(req, res)) {
            ...
            if (user) {
                ...
                if (profile) {
                    const deleted = await BankModel.mobile.delete(profile.unique, data.unique);
                    if (deleted) {
                        Response(res, { message: "The mobile bank has been successfully deleted." }, 200);
                    } else {
                        Response(res, { message: "Failed to delete mobile bank" }, 500);
                    }
                } else {
