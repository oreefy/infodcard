    ...
    try {
        if (await method(req, res)) {
            ...
            if (user) {
                ...
                const profiles = author ? await ProfileModel.find.authorProfiles(author.unique, "profile.cover profile.visitor profile.avatar profile.bio profile.youtube profile.phone profile.profession profile.website profile.company profile.companyphone profile.companyemail profile.designation profile.corporate profile.branch profile.email profile.address profile.qr profile.socials profile.groupName profile.groupItems profile.products profile.albums profile.services profile.localBanks profile.mobileBanks profile.globalBanks") : [];
