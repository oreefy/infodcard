...
export default async function UpdateService(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        if (await method(req, res)) {
            ...
            if (user) {
                ...
                if (find) {
                    const update = await ServiceModel.update(find.unique, data.unique, data.title, data.price, data.message);
                    if (update) {
                        Response(res, { errors: [{ type: "other", success: true, message: "The service has been successfully updated." }] }, 200)
                    } else {
                        Response(res, { message: "Faield to update a service" }, 500);
                    }
                } else {
