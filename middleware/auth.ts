...
export async function Admin(req: NextApiRequest, res: NextApiResponse): Promise<{ name: string; email: string; image: string } | null> {
    try {
        if (await method(req, res)) {
            const user = await getServerSession(req, res, authOptions)
            if (user) {                
                if (user.user?.email === "infodcardbd@gmail.com") {
